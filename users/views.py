...
def deposit_window(request):
    ...
    if request.method == "POST" and user.is_authenticated:
        ...
        if not pending_hash == True and not price_total:
            ...
            _hash = qs.hash_id
            qs.msg = f"Total Price at ${price_total} is being processed"
            transactionId = qs.transactionId
            qs.save()
        else:
            qs = Transaction.objects.create(
                ...
                status="processing",
                transactionId=random_string_generator(size=17).upper(),
                msg=f"${price_total} is being processed",
            )
            qs.save()
            _hash = qs.hash_id
            transactionId = qs.transactionId
    ...
    if request.method == "POST":
        ...
        if not transactionId == True and not pending_hash == None:
            ...
            if not qs is None and not price == None:
                ...
                qs.msg = f"You deposit request of ${price} is being confirmed"
                qs.save()
                # btc_address = wallet.btc_address
                ...
                if qsr is None:
                    ...
                    try:
                        ...
                        send_alert_mail(
                            ...
                            email_message=f"User `{user.username}` deposited ${price} and its undergoing confirmation by the team. It will take 1-3 days before he/she gets credited [{user.username} wallet address is `{wallet.btc_address}`]",
...
def withdraw_window(request):
    ...
    if request.method == "POST" and user.is_authenticated:
        ...
        if not pending_hash == True and not price:
            ...
            qs.status = "processing"
            _hash = qs.hash_id
            qs.msg = f"Withdrawal of ${price} is being processed"
            transactionId = qs.transactionId
            qs.save()
        else:
            qs = Transaction.objects.create(
                ...
                status="processing",
                transactionId=random_string_generator(size=17).upper(),
                msg=f"${price} withdrawal is being processed",
            )
            qs.save()
            _hash = qs.hash_id
            transactionId = qs.transactionId
